...
if ($null -eq $group) { Write-Error "A team with the mailNickname $groupMailNickname was not found" }
else{
  ...
  if ($null -eq $channels) { Write-Error "A channel with the name $channelName was not found in the team" }
    ...
    $tabs = o365 teams tab list --teamId $group[0].id --channelId $channel[0].id --query "[?teamsApp.id=='com.microsoft.teamspace.tab.wiki']" -o json | ConvertFrom-Json
    if ($null -eq $tabs) { Write-Error "A Wiki tab was not found in the channel" }
      ...
      o365 teams tab remove --teamId $groups[0].id --channelId $channel[0].id --tabId $tabs[0].id --confirm
